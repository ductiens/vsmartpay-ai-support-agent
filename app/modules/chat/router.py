...
async def get_sessions(
    ...
    """
    return success_response(data=await chat_service.get_user_sessions(current_user.user_id))

@router.get("/chat/sessions/{session_id}/history", response_model=BaseSuccessResponse[List[ChatMessageResponse]])
async def get_session_history(
    ...
    """
    return success_response(data=await chat_service.get_user_session_history(session_id, current_user.user_id))


# ──────────────────── CSKH Dashboard (Admin) APIs ────────────────────

@router.get("/api/v1/admin/chat-sessions/waiting", response_model=BaseSuccessResponse[list])
...
@router.get("/api/v1/admin/chat-sessions/{session_id}/messages", response_model=BaseSuccessResponse[List[ChatMessageResponse]])
async def admin_get_session_history(
    ...
    """
    return success_response(data=await chat_service.admin_get_session_history(session_id))

@router.post("/api/v1/admin/chat-sessions/{session_id}/messages", response_model=BaseSuccessResponse[dict])
